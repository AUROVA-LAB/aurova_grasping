...
class ImpedanceController:
    def __init__(self):
        ...
        self.force_desired = rospy.get_param('force_desired',  1.0)  # Desired force in N
        self.K_p = rospy.get_param('K_p', 0.0010)  # Proportional gain (0.2 for solid surface)
        self.K_d = rospy.get_param('K_d', 0.00015)  # Derivative gain (0.0001-0.00025 for solid surface)

        self.K_p_orient = rospy.get_param('K_p_orient',0.5)  # Gain for orientation correction
        ...
        self.radius = rospy.get_param('circle_radius', 0.04)  # [m]
    ...
    def control_loop(self):
        ...
        if(not(self.contact)and not (self.init)):
            ...
            if (reference_frame == 2):
                ...
                twist_cmd.twist.linear_z = 0.0025 
        ...
        # ---- Main experiment loop ----
            ...
            Fx = 1.25*self.forces[3]["fZ"] - self.forces[5]["fZ"]  # Force difference in Y
            Fy = self.forces[7]["fZ"] - 1.35*self.forces[1]["fZ"]  # Force difference in X
            ...
            if (reference_frame == 2):
                twist_cmd.twist.linear_x = 0*(vy_cmd + vy_f)  # should be positive in Y
                twist_cmd.twist.linear_y = 0*(vx_cmd + vx_f)
                ...
                twist_cmd.twist.linear_x = 0*vx_cmd + vx_f
